...
<header id="header"
    ...
    <div class="flex relative gap-2 items-center pr-4">
        <button id="notif-btn" data-dropdown-toggle="notif-ddw"
            class="text-sm p-2 text-gray-700 hover:bg-gray-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg dark:text-gray-400 dark:hover:text-white dark:focus:ring-gray-800">
            @empty($panel->user->unreadNotifications->all())
            @else
        ...
        <button id="lang-btn" data-dropdown-toggle="lang-dropdown"
            ...
            <div class="p-2.5">
                <div class="font-semibold uppercase">{{ $panel->locale }}</div>
            </div>
        </button>
                ...
                @foreach ($panel::$locales as $key => $value)
                    <li>
                        <a href="{{ route('web.locale.set', ['locale' => $key]) }}" @class([
                            'block py-2 px-4 capitalize hover:text-gray-900 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white',
        ...
        <button id="ProfileButton" data-dropdown-toggle="Profile" data-tooltip-target="ProfileButtonTooltip"
            ...
            <div class="hidden md:block md:max-w-[120px] text-left font-medium dark:text-white">
                <div class="text-ellipsis truncate text-base">{{ $panel->get_user_name() }}</div>
                <div class="text-ellipsis truncate text-xs opacity-70">{{ $panel->get_user_identifier() }}</div>
            </div>
            <svg class="hidden md:block w-4 h-4 mx-1.5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
        ...
        <div id="ProfileButtonTooltip" role="tooltip"
            class="block md:hidden absolute z-10 invisible px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
            Profile
            <div class="tooltip-arrow" data-popper-arrow></div>
        </div>
        <div id="Profile"
            class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            <div class="block md:hidden py-3 px-4 text-sm text-gray-900 dark:text-white">
                <div class="truncate text-ellipsis font-semibold">{{ $panel->get_user_name() }}</div>
                <div class="truncate text-ellipsis ">{{ $panel->get_user_identifier() }}</div>
            </div>
            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                ...
                <li>
                    <button id="theme_menu_btn" data-dropdown-toggle="theme_menu"
                        ...
                        class="flex sm:hidden items-center justify-between w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        {{ trans('thema') }}
