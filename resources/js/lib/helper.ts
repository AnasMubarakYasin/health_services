  ...
  const handler = (event) => {
    ...
    if (typeof img_id == "string") {
      ...
      if (img.nodeName != "IMG") {
        img.replaceWith(
          create_element(
            `<img id="${img_id}" class="w-[9rem] aspect-square object-cover object-center text-gray-400" src="${input_img_preview_data[input_id]}" alt="">`
          )
        );
      } else {
...
export function toggle_button_group(
  group: string,
