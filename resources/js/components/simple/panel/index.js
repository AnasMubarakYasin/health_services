...
for (const themeItem of themeList.children) {
  themeItem.firstElementChild.addEventListener("click", () => {
    localStorage.setItem(
      "theme",
      themeToggle(
        themeItem.firstElementChild.dataset.theme == "light" ? "dark" : "light"
      )
    );
  });
}
function themeSelect(theme) {
  ...
  themeList
    .querySelector(`[data-value="${theme}"]`)
    .classList.add(...themeClass);
}
function themeToggle(theme) {
  const btn = themeList.querySelector(`[data-value="${theme}"]`);
  ...
  if (theme == "light") {
    ...
    document.documentElement.classList.add("dark");
    themeBtn.innerHTML = icon_light;
    themeList.querySelector(`[data-value="dark"]`).classList.add(...themeClass);
    return "dark";
  } else if (theme == "dark") {
    document.documentElement.dataset.theme = "light";
    document.documentElement.classList.remove("dark");
    themeBtn.innerHTML = icon_dark;
    themeList
      .querySelector(`[data-value="light"]`)
      .classList.add(...themeClass);
    return "light";
  } else {
    alert("something wrong");
